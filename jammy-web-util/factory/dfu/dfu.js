...
dfu.Device.prototype.flashState = function (state) {
...
dfu.Device.prototype.do_download = async function (xfer_size, data, manifestationTolerant) {
    ...
    this.logInfo("dfu: do_download");
    this.logInfo("Copying data from browser to DFU Jammy device");
    ...
    while (bytes_sent < expected_size) {
        ...
        try {
            ...
            this.logDebug("Sent " + bytes_written + " bytes of " + expected_size);
            dfu_status = await this.poll_until_idle(dfu.dfuDNLOAD_IDLE);
            this.logDebug("Send status: " + dfu_status);
        } catch (error) {
        ...
        this.logDebug("Wrote " + bytes_written + " bytes of " + expected_size);
    ...
    this.logInfo("Wrote " + bytes_sent + " bytes of " + expected_size);

...
dfuse.Device.prototype.dfuseCommand = async function (command, param, len) {
    ...
    if (status.status !== dfu.STATUS_OK) {
        throw new Error("Special DfuSe command " + commandNames + " failed");
    }
};
...
dfuse.Device.prototype.erase = async function (startAddr, length) {
    ...
    while (addr < endAddr) {
        ...
        this.logDebug(`Erased ${bytesErased}B`);
        this.logProgress(bytesErased, bytesToErase);
    }
};
...
dfuse.Device.prototype.do_download = async function (xfer_size, data, manifestationTolerant) {
    ...
    this.flashState("erase");
    await this.erase(startAddress, expected_size);

    this.flashState("download");

    this.logInfo("Copying data from browser to DFU device");

    let address = startAddress;
    var full_bytes_written = 0
    while (bytes_sent < expected_size) {
        ...
        try {
            ...
            this.logDebug("Sent " + bytes_written + " it's " + full_bytes_written + " bytes of " + expected_size);
            dfu_status = await this.poll_until_idle(dfu.dfuDNLOAD_IDLE);
            this.logDebug("Sent status: " + JSON.stringify(dfu_status));
            address += chunk_size;
        } catch (error) {
...
dfuse.Device.prototype.do_leave = async function () {
    console.log("Do leave.")
    try {
        var startAddress = 0x0800f000;
        console.log("Address: ", this.startAddress)
        if (isNaN(startAddress)) {
            startAddress = this.memoryInfo.segments[0].start;
        }
        console.log("Address st: ", this.startAddress)
        let state = await this.getState();
        if (state !== dfu.dfuIDLE) {
            await this.abortToIdle();
        }
        ...
        await this.dfuseCommand(dfuse.SET_ADDRESS, startAddress, 4);
        await this.abortToIdle();
        console.log("Set: ", this.startAddress)
        await this.download(new ArrayBuffer(), 0);
        await this.poll_until(state => (state === dfu.dfuMANIFEST));
    } catch (error) {
        console.log("Address er: ", this.startAddress)
        this.logError("Error during DfuSe do_leave: " + error);
    }
}
